from django.contrib import admin
from .models import *
# Register your models here.


admin.site.register(Department)
admin.site.register(Banner)
admin.site.register(AvailableTime)
admin.site.register(Doctor)
admin.site.register(Gallery)
admin.site.register(Blog)
admin.site.register(Message)
admin.site.register(Career)
admin.site.register(CareerApplication)
admin.site.register(BlogComment)
admin.site.register(MonthlyTiming)
admin.site.register(Leave)
admin.site.register(Appointment)
admin.site.register(Patient)
admin.site.register(RazorpayPaymentDetails)
admin.site.register(Notification)
admin.site.register(HealthCheckupPlan)
admin.site.register(HealthCheckupBooking)
admin.site.register(Summary)
admin.site.register(HeartDayCheckup)
admin.site.register(CheckupDescription)
admin.site.register(HeartDayCheckupBooking)
admin.site.register(DepartmentAppointment)
admin.site.register(PatientProfile)